...
public class NumericArithmetic {
    ...
    public static void checkInputBoundary(Literal input, double lowerBound, double upperBound,
        ...
        if (input instanceof DoubleLiteral) {
            ...
            if (lowerCheck || upperCheck) {
                throw new AnalysisException("input of " + input.toSql() + "is out of boundary");
            }
        }
    }
    ...
    private static Expression checkOutputBoundary(Literal input) {
        if (input instanceof DoubleLiteral) {
            if (((DoubleLiteral) input).getValue().isNaN()) {
                throw new AnalysisException("result of " + input.toSql() + "is NaN");
            }
            if (((DoubleLiteral) input).getValue().isInfinite()) {
                throw new AnalysisException("result of " + input.toSql() + "is infinite");
            }
        }
        return input;
    }
    ...
    public static Expression sin(DoubleLiteral first) {
        checkInputBoundary(first, Double.MIN_VALUE, Double.MAX_VALUE, true, true);
